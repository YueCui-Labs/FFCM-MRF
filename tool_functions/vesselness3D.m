...
for j = 1:length(sigmas)
    ...
    clear Lambda2 Lambda3 Lambda3M 
    %keep max response 寻找最大响应
    if(j==1)
        vesselness = response;
        save vesselness.mat vesselness -v7.3
        clear response vesselness
    else
        vesselness = importdata('vesselness.mat');
...
% calculates the three eigenvalues for each voxel in a volume计算单位体积下每个体素的特征值

% Calculate 3D hessian得到3维hessian矩阵
[Hxx, Hyy, Hzz, Hxy, Hxz, Hyz] = Hessian3D(V,sigma,spacing);

% Correct for scaling缩放比例校正
c=sigma.^2;
Hxx = c*Hxx; Hxy = c*Hxy;
Hxz = c*Hxz; Hyy = c*Hyy;
Hyz = c*Hyz; Hzz = c*Hzz;
...
% S.-F. Yang and C.-H. Cheng, ?ast computation of Hessian-based
...
% Free memory
clear Lambda1i Lambda2i Lambda3i T;
...
% Create first and second order diferentiations构造二阶导数，得到的是和原图像大小一致的3维矩阵
Dz=gradient3(F,'z');
Dy=gradient3(F,'y');
Dx=gradient3(F,'x');
clear F;
...
if(sigma>0)
    ...
    % Filter each dimension with the 1D Gaussian kernels\每个维度对应使用1维高斯核
    x=-ceil(siz/spacing(1)/2):ceil(siz/spacing(1)/2);
    H = exp(-(x.^2/(2*(sigma/spacing(1))^2)));
    H = H/sum(H(:));    
    Hx=reshape(H,[length(H) 1 1]);%自制高斯模板
