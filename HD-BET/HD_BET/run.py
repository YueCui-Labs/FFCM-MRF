...
def apply_bet(img, bet, out_fname):
    ...
    out_float = sitk.Cast(out, sitk.sitkFloat32)
    sitk.WriteImage(out_float, out_fname)
    

def run_hd_bet(mri_fnames, output_fnames, mode="fast", config_file=os.path.join(HD_BET.__path__[0], "config.py"), device=0,
               postprocess=False, do_tta=False, keep_mask=True, overwrite=True):
    """
...
#     print('runs here, above load_source')
    cf = imp.load_source('cf', config_file)
    cf = cf.config()

#     print('runs here, above get_network')
