...
class ExtractDiameter:
    ...
    def run(self):
        ...
        if self._distance_output:
            nib.save(nib.Nifti1Image(edt, img.affine,
                                     img.header), self._distance_output)

        ridge_distance = compute_ridge_distance(edt)

        if self._ridge_output:
            nib.save(nib.Nifti1Image(ridge_distance, img.affine,
                                     img.header), self._ridge_output)

        thickness = compute_thickness(ridge_distance)

        if self._thickness_output:
            nib.save(nib.Nifti1Image(thickness * data, img.affine,
                                     img.header), self._thickness_output)

        diameter_clean = compute_clean_thickness(data, thickness, pix_dim[0])

        nib.save(nib.Nifti1Image(diameter_clean, img.affine,
                                 img.header), self._output)
